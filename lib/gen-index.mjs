...
function renderTasks(dir) {
  const items = siblings(dir);
  const known = { active: [], done: [], cancelled: [] };
  const other = [];
  for (const it of items) { if (known[it.status]) known[it.status].push(it); else other.push(it); }
  const sec = (label, arr) => arr.length ? `**${label}（${arr.length}）**\n` + arr.map((it) => `- [${it.title}](${it.file})`).join('\n') : '';
  const parts = [sec('进行中', known.active), sec('已完成', known.done), sec('已取消', known.cancelled), sec('其它', other)].filter(Boolean);
