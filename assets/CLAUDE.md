...
本文件是项目根级 Claude Code 路由总则。长期知识放 `docs/`，短期运行状态放 `.ai/`，不要把两者混写。
...
4. 若进入子项目，继续读取更近的 `CLAUDE.md` / `AGENTS.md`。
...
- 新建任务：`/gq-code-harness:new-task`
- 归档任务：`/gq-code-harness:archive`
- 沉淀任务：`/gq-code-harness:distill`
- 固化检查：`/gq-code-harness:contract`
- 临时放行：`/gq-code-harness:override`
- 重新铺底：`/gq-code-harness:init`

## 验证

- 修改文档后，依赖 gq-code-harness 的 Stop hook / doctor 检查死链。
