...
```bash
codex plugin marketplace add noug12138/gq-code-harness
codex plugin add gq-code-harness@gq-harness-marketplace
```

装完后开新 thread 生效。Codex 走 `.codex-plugin/plugin.json`、`hooks/hooks.json` 和 `skills/`；Claude Code 仍走 `.claude-plugin/`、`commands/` 和同一套 hook 脚本。
...
Codex 下 `check-runner` 兼容 `apply_patch`；对无法可靠检查最终内容的 shell 写入，会要求改用 `apply_patch` / Write 路径。
...
Codex 不直接使用 Claude slash command 文件；等价入口是 skills：

| Codex skill | 对应 Claude 命令 |
| --- | --- |
| `$gq-contract` | `/gq-code-harness:contract` |
| `$gq-override` | `/gq-code-harness:override` |
| `$gq-new-task` | `/gq-code-harness:new-task` |
| `$gq-archive` | `/gq-code-harness:archive` |
| `$gq-distill` | `/gq-code-harness:distill` |
| `$gq-init` | `/gq-code-harness:init` |

## Skill

核心方法论 skill 是 `harness-lifecycle`，是单文件任务建、做、归档、沉淀的方法论。在 .ai/tasks/ 下干活、或被上面那三个任务命令调用时加载。

Codex 额外提供 `gq-*` 包装 skills，把 Claude command 工作流暴露成 Codex 可调用流程，但不删除原 command 文件。
...
第一步铺骨架，是确定性的、不用 agent：把根 `AGENTS.md` / `CLAUDE.md`、`.ai/` 和 `docs/` 的目录架子复制过去，已经存在的文件跳过。
...
```
项目根/
├─ AGENTS.md                  Codex / 通用 agent 路由总则
├─ CLAUDE.md                  Claude Code 路由总则
├─ docs/                      长期知识
